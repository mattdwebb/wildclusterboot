...
test_that('wild_cluster_boot returns correct value',{
  ...
  data <- read.csv('/Users/scott/GitHub/wildclusterboot/tests/testthat/test_files/test_boot_p.csv')
  ...
  test_p <- wild_cluster_boot(data = data, model = model, x_interest = x_interest, clusterby = clusterby,
                              boot_dist = boot_dist, boot_reps = boot_reps)

  expect_equal(test_p, 0.66667, tolerance = 0.0001)
