...
test_that('calc_cluster_se calculates proper values',{
  ...
  expect_equal(se, test_se, tolerance = 0.002)
...
test_that('clustered_se calculates proper values',{
  ...
  model <- lm(Y ~ X, x = TRUE)
  se <- clustered_se(model = model, clusterby = clusterby)
  expect_equal(se, test_se, tolerance = 0.002)
  expect_equal(length(se),2)

  #Check that se works with cluster_ids and G
  X1 <- cbind('(Intercept)' = 1, X)
  model <- lm(Y ~ X)
  se <- clustered_se(model = model, clusterby = clusterby, cluster_ids = c(1,2), G = 2)
  expect_equal(se, test_se, tolerance = 0.002)
  ...
  expect_error(clustered_se(model = '', clusterby = clusterby),
